...
import { ChevronLeft, Volume2, Send, Square } from 'lucide-react';
import MobileShell from '../components/MobileShell';
import { cn } from '../lib/utils';
import { speakText, stopSpeak, isTTSAvailable } from '../lib/useSpeech';
    ...
    <MobileShell className="bg-gradient-to-b from-amber-50 via-yellow-50 to-white">
      ...
      <div className="px-6 pt-4 flex items-center justify-center sticky top-0 bg-gradient-to-b from-amber-50/90 to-transparent z-10 pb-2">
        ...
        <div className="flex flex-col items-center">
          ...
          <span className={cn(
            ...
            isSpeaking ? 'bg-amber-100 text-amber-600' : 'bg-sky-100 text-sky-500'
          )}>
          {isSpeaking ? '正在朗读' : profile.name}
          </span>
        </div>
      </div>
      ...
      <div className="px-4 pb-4 pt-2 bg-gradient-to-t from-white to-amber-50/50 rounded-t-[32px] shadow-[0_-4px_20px_rgba(0,0,0,0.08)] space-y-4 safe-area-bottom">
        ...
        <div className="w-full flex items-center gap-2 px-1">
          ...
          <button
            disabled
            className="w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 bg-amber-100 opacity-50"
          >
